...
public class ApiAspect {
    ...
    public Object aroundMethod(ProceedingJoinPoint pjp, RequestMapping requestMapping) {
        ...
        if (CollectionUtil.isNotEmpty(parameterMap)) {
            logger.info(logDetail.setTag(request.getRequestURI()).setStage("接收参数")
                    .setMsg("request=" + JSON.toJSONString(parameterMap)).toString());
        }
        ...
        try {
            ...
            logger.error(logDetail.setMsg("类=%s,方法=%s,异常信息=%s", pjp.getSignature().getDeclaringTypeName(),
                    pjp.getSignature().getName(), throwable.getMessage()).toString());
