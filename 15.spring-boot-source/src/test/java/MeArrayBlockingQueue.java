...
public class MeArrayBlockingQueue<E> extends AbstractQueue<E>
	 ...
	 * @throws IllegalArgumentException if {@code capacity < 1}
	 */
	public MeArrayBlockingQueue(int capacity, boolean fair) {
		if (capacity <= 0)
	...
	private class Itr implements Iterator<E> {
		...
		Itr() {
			...
			lastRet = NONE;
			final ReentrantLock lock = MeArrayBlockingQueue.this.lock;
			lock.lock();
			try {
				if (count == 0) {
					// assert itrs == null;
					...
					final int takeIndex = MeArrayBlockingQueue.this.takeIndex;
