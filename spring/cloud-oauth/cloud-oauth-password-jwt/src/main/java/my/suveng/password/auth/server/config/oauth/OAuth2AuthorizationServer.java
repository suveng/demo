...
public class OAuth2AuthorizationServer extends AuthorizationServerConfigurerAdapter {
    ...
    /**
     * 用户认证器,用户源在{@link org.springframework.security.core.userdetails.UserDetailsService}的实现类中找到 实现类为:
    ...
    public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
        endpoints.userDetailsService(userDetailsService)// 配置用户认证来源,可扩展用户
                .tokenStore(tokenStore())// 配置认证器的token存储方式
                // 配置用于JWT私钥加密的增强器
                .tokenEnhancer(jwtTokenEnhancer()).authenticationManager(authenticationManager);// 配置认证器
    }
