...
public class EventHandler {
    ...
    private final ThreadPoolExecutor executor = new ThreadPoolExecutor(4, 10, 60L, TimeUnit.SECONDS, workQueue,
            ThreadFactoryBuilder.create().setNamePrefix("exception-handler-").build(),
    ...
    public Result defaultErrorHandler(HttpServletRequest req, Exception e) {
        ...
        executor.execute(() -> {
            Robot.build(RobotEnums.TEXT_ROBOT).init(dingTalkMessage.getLink()).sendMsgAt(dingTalkMessage.toBeautiful(),
                    dingTalkMessage.getPhone());
        });
